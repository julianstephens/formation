...
var phaseChangeMessages = map[domain.SeminarSessionPhase]string{
...
func InsertPhaseChangeTurn(
	ctx context.Context,
	r *repo.SeminarSessionRepo,
	sessionID string,
	toPhase domain.SeminarSessionPhase,
) (*domain.SeminarTurn, error) {
	text, ok := phaseChangeMessages[toPhase]
	if !ok {
		text = fmt.Sprintf("Phase advanced to %s.", toPhase)
	}

	t := domain.SeminarTurn{
		SessionID: sessionID,
		Phase:     toPhase,
		Speaker:   "system",
		Text:      text,
